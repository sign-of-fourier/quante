...
class hp_tuning_session:
    ...
    def test_new_points(self, p, other_parameters=None):

        if other_parameters:
            self.scores = p.map(self.model, self.next_points)
        else:
            self.scores = p.map(self.model, [{'next_points': next_points[i], 'ohter_parameters': i} for i in range(self.n_processors)])
...
def hp_tune(f, limits, kernel, n_batches, n_processors, n_iterations):
    def qc_tune_nn(p):
